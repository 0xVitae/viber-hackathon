...
export function TherapyChat({ onBack }: TherapyChatProps) {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-orange-50 to-white">
      ...
      <header className="sticky top-0 z-40 border-b-4 border-black bg-orange-500 px-4 py-3 shadow-[0_4px_0_#000]">
        <motion.div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="font-display text-xl text-white drop-shadow-[2px_2px_0_#000] sm:text-2xl">
              Temu Therapist
            </h1>
            <p className="text-xs font-bold text-yellow-200">Healing. But affordable.</p>
          </div>
          <CountdownTimer />
          <motion.div
            className="flex items-center gap-2 rounded-full border-2 border-black bg-white px-4 py-2 font-black"
      ...
      <div className="mx-auto flex max-w-6xl gap-4 px-4 py-4">
        ...
        <main className="min-w-0 flex-1">
          ...
          <div
            className={`mt-4 flex h-[calc(100vh-280px)] min-h-[400px] flex-col rounded-2xl border-4 border-black bg-white shadow-[6px_6px_0_#000] ${
              chaosLevel >= 3 ? "animate-pulse-glow" : ""
            }`}
          >
            <motion.div className="flex-1 overflow-y-auto p-4 scrollbar-hide">
              <AnimatePresence>
                {messages.map((msg) => (
                  <motion.div
                    key={msg.id}
            ...
            {userMessageCount === 0 && (
              <div className="flex flex-wrap gap-2 border-t-2 border-orange-100 px-4 py-2">
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => sendMessage(s)}
                    className="rounded-full border-2 border-orange-400 bg-orange-100 px-3 py-1 text-xs font-bold text-orange-800 hover:bg-orange-200"
                  >
                    {s}
                  </button>
                ))}
            ...
            <form
              className="flex gap-2 border-t-4 border-black p-4"
              onSubmit={(e) => {
                e.preventDefault();
                sendMessage(input);
              }}
            >
              <input
                ...
                className="flex-1 rounded-xl border-4 border-black px-4 py-3 font-semibold outline-none focus:ring-4 focus:ring-neon-green/50"
                disabled={typing}
              />
              <motion.button
                type="submit"
                disabled={typing || !input.trim()}
                className="flex items-center gap-2 rounded-xl border-4 border-black bg-neon-green px-5 py-3 font-black uppercase disabled:opacity-50"
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              ...
              className="animate-float rounded-lg border-2 border-black bg-gold px-4 py-2 text-xs font-black uppercase shadow-[3px_3px_0_#000]"
            >
              🎡 Spin Wheel
            </button>
            <button
              ...
              className="rounded-lg border-2 border-black bg-purple-400 px-4 py-2 text-xs font-black uppercase text-white shadow-[3px_3px_0_#000]"
            >
              <Gift className="inline h-4 w-4" /> Therapy Bundle
            </button>
            <button
              type="button"
              onClick={onBack}
              className="rounded-lg border-2 border-gray-400 px-4 py-2 text-xs font-bold text-gray-600"
            >
              <RotateCcw className="inline h-4 w-4" /> Escape
            </button>
          </div>
        </main>
      </div>
      ...
      {chaosLevel >= 4 && (
        <motion.button
          type="button"
          className="fixed bottom-4 left-4 z-30 animate-shake rounded-full border-4 border-black bg-red-600 px-6 py-4 font-black text-white shadow-lg"
          onClick={() => setShowWheel(true)}
        >
          SPIN NOW
        </motion.button>
